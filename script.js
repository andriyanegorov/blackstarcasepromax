/* ==============================================
   SCRIPT.JS - FULL UPDATE (Drop Day + Fair Upgrades)
   ============================================== */

// --- GLOBAL VARIABLES & STATE DEFINITIONS ---
const STORAGE_KEY = 'br_cases_user_v2'; // Key for saving data
const DEFAULT_USER = {
    uid: 0,
    name: "Игрок",
    gameNick: "",
...
let user = JSON.parse(JSON.stringify(DEFAULT_USER)); // Initialize user
...
// ⚠️ ВСТАВЬТЕ СЮДА ВАШ НОВЫЙ URL ОТ GOOGLE SCRIPT ⚠️
...
/* ==============================================
   КОНФИГУРАЦИЯ ИГРЫ (Полная копия ваших данных)
   Note: Changed to 'let' to allow overwriting from Admin Panel
   ============================================== */
let GAME_CONFIG = [
    {
        "id": "sub_case_1", "name": "Раз в ДВА ДНЯ! (За подписку)", "price": 0, "category": "free", "img": "img/free_case.png",
    ...
    {
        "id": 1, "name": "Кейс Бомжа", "price": 29, "category": "default", "img": "img/bomj_case.png",
    ...
    // ... (Остальные кейсы оставлены без изменений, но убедитесь, что структура валидна)
    {
        "id": 1770544149013, "name": "Стандартный Кейс", "price": 199, "category": "default", "img": "img/standart_case.png",
    ...
    // ... Add all other cases here as in your original file ...
    // Для краткости я использую вашу структуру, но переменная объявлена через LET
];
...
async function loadDailyDrop() {
    const container = document.getElementById('daily-drop-content');
    if(!document.getElementById('daily-drop-card')) return;

    try {
        if(API_URL.includes("ВСТАВЬТЕ")) {
             document.getElementById('daily-drop-card').style.display = 'none';
             return;
        }

        const res = await fetch(`${API_URL}?action=get_best_drop`);
        const data = await res.json();

        if (data.status && data.data) {
            const drop = data.data;
            document.getElementById('dd-user').innerText = drop.nick || "Аноним";
            document.getElementById('dd-item-name').innerText = drop.item;
            document.getElementById('dd-price').innerText = drop.price + " ₽";
            document.getElementById('dd-img').src = drop.img;
            document.getElementById('daily-drop-card').style.display = 'block';
        } else {
            document.getElementById('daily-drop-card').style.display = 'none';
        }
    } catch(e) {
        console.error("Failed to load daily drop", e);
        document.getElementById('daily-drop-card').style.display = 'none';
    }
}

function checkAndRecordDrop(item) {
    if (item.price >= 500 && !API_URL.includes("ВСТАВЬТЕ")) {
        const query = `action=record_drop&uid=${user.uid}&nick=${encodeURIComponent(user.name)}&item=${encodeURIComponent(item.name)}&price=${item.price}&img=${encodeURIComponent(item.img)}`;
        fetch(`${API_URL}?${query}`, { mode: 'no-cors' }).catch(e => console.log(e));
    }
}
...
function addHistory(text, val) { 
    const color = val.toString().includes('+') ? '#4CAF50' : '#ff4d4d'; 
    user.history.unshift({ text, val, color }); 
...
function finishWin(keep) {
    ...
    currentWins.forEach(i => {
        ...
        checkAndRecordDrop(i); // Записываем если дроп дорогой
    });
...
// --- UPGRADE SYSTEM (REWRITTEN: FAIR CHANCES, NERFED X2) ---
...
function selectUpgradeSource(idx) { 
    upgradeState.sourceIdx = idx; 
    const item = user.inventory[idx]; 
    document.getElementById('upg-source-slot').querySelector('.placeholder-icon').style.display = 'none'; 
    const img = document.getElementById('upg-source-img'); 
    img.src = item.img; 
    img.style.display = 'block'; 
    const pr = document.getElementById('upg-source-price'); 
    pr.innerText = item.price + '₽'; 
    pr.style.display = 'block'; 
    closeModal('modal-upg-select'); 
    updateUpgradeCalculation(); 
}

function setUpgradeMultiplier(m) { 
    let ch = Math.floor(100/m); 
    if(ch > 75) ch = 75; 
    if(ch < 1) ch = 1; 
    document.getElementById('upg-chance-slider').value = ch; 
    updateUpgradeCalculation(); 
}
...
function updateUpgradeCalculation() { 
    ...
    const chance = parseInt(document.getElementById('upg-chance-slider').value); 
    upgradeState.chance = chance; 
    
    document.getElementById('upg-chance-display').innerText = chance + '%'; 
    document.getElementById('roll-win-zone').style.width = chance + '%'; 
    
    const srcPrice = user.inventory[upgradeState.sourceIdx].price; 
    
    // Рассчитываем множитель из шанса: (100 / шанс)
    const multiplier = 100 / chance;
    const targetPrice = Math.floor(srcPrice * multiplier); 

    let best = null; 
    for(let i of ALL_ITEMS_POOL) { 
        // Ищем предмет близкий к targetPrice, но не дешевле исходного (с небольшим запасом 10% вверх)
        if(i.price > srcPrice && i.price <= targetPrice * 1.1) { 
            if(!best || i.price > best.price) best = i; 
        } 
    } 
    
    const content = document.getElementById('upg-target-content'); 
    const notFound = document.getElementById('upg-not-found'); 
    const ph = document.getElementById('upg-target-placeholder'); 
    const btn = document.getElementById('btn-do-upgrade'); 
    
    ph.style.display = 'none'; 
    
    if(best) { 
        upgradeState.targetItem = best; 
        content.style.display = 'block'; 
        notFound.style.display = 'none'; 
        document.getElementById('upg-target-img').src = best.img; 
        document.getElementById('upg-target-price').innerText = best.price + ' ₽'; 
        btn.disabled = false; 
        
        // Предупреждение о порезке шанса для x2 (50%)
        const statusText = document.getElementById('upg-status-text');
        if (chance === 50) {
            statusText.innerText = "Множитель x2: Шанс успеха снижен до 25%!";
            statusText.className = "status-text status-lose";
        } else {
...
function startUpgrade() { 
    ...
    // --- ЧЕСТНЫЕ ШАНСЫ (С НЕРФОМ X2) ---
    let winThreshold = upgradeState.chance;
    
    // Если игрок выбрал 50% (x2), режем шанс в 2 раза
    if (winThreshold === 50) {
        winThreshold = 25; 
        console.log("X2 Modifier applied: Chance reduced to 25%");
    }
    ...
    // Визуализация: Куда упадет стрелка
    let visualRoll;
    if (isWin) {
        // Успех: стрелка падает в зону от 0 до visualChance (зеленая зона)
        visualRoll = Math.random() * upgradeState.chance;
    } else {
        // Проигрыш: стрелка падает от visualChance до 100 (черная зона)
        visualRoll = upgradeState.chance + (Math.random() * (100 - upgradeState.chance));
        if (visualRoll > 98) visualRoll = 98; // Граница
    }
...
function resetUpgradeUI() { 
    upgradeState.sourceIdx = null; 
    document.getElementById('upg-source-img').style.display = 'none'; 
    document.getElementById('upg-source-price').style.display = 'none'; 
    document.getElementById('upg-source-slot').querySelector('.placeholder-icon').style.display = 'block'; 
    document.getElementById('upg-target-content').style.display = 'none'; 
    document.getElementById('upg-target-placeholder').style.display = 'block'; 
    document.getElementById('upg-not-found').style.display = 'none'; 
    document.getElementById('roll-pointer').style.transition = 'none'; 
    document.getElementById('roll-pointer').style.left = '0%'; 
    document.getElementById('upg-status-text').innerText = ''; 
    document.getElementById('btn-do-upgrade').disabled = true; 
}