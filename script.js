...
// --- STATE ---
const STORAGE_KEY = 'br_user_data_v5';
const DEFAULT_USER = { 
    balance: 0, 
    inventory: [], 
    uid: 0, 
...
/* ==============================================
   INIT (ОБРАБОТКА ОШИБОК И ЗАГРУЗКА)
   ============================================== */
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // Загружаем конфиги и пользователя
    loadExternalConfig(); 
    initCases(); 
    flattenItems();
    initUserSession();
...
function loadFromLocal() {
    ...
    if (localData) {
        try {
            ...
            // FIX: Принудительное преобразование баланса в число, чтобы избежать строковой конкатенации
            user.balance = Number(user.balance); 
...
/* --- OPEN CASE PREVIEW & CHECK SUB LOGIC --- */
...
function openPreview(id) { 
    try {
        ...
        // Восстанавливаем структуру кнопки
        ...
        if(selectedCase.category === 'free') {
            qtySel.style.display = 'none'; // Only open x1
...
async function verifySubscription() {
    try {
        const response = await fetch(`https://api.telegram.org/bot${BOT_TOKEN}/getChatMember?chat_id=${CHAT_ID}&user_id=${user.uid}`);
...
function setOpenCount(n) { 
    ...
    // Обновляем кнопки выбора количества
...
async function startRouletteSequence() {
    try {
        if(selectedCase.category === 'free') {
            ...
            if(!isSub) {
                btn.disabled = false;
                btn.innerText = "ОТКРЫТЬ БЕСПЛАТНО";
                btn.style.display = 'none';
                subBtn.style.display = 'block'; 
                safeAlert("Вы не подписаны на канал! Подпишитесь для открытия.");
                return;
            }
...
// --- FIX: Активация промокода (Принудительно Number) ---
function activatePromo() { 
    ...
    if(p) { 
        ...
        // ВАЖНО: Принудительное преобразование в числа
